...
- **Streak**: consecutive fast outcomes within a drill; a slow or missed outcome resets it. The best streak of a drill is shown on its end screen. A streak does not carry across drills; gems are the one thing that does.
...
- **Gem**: what the learner earns and keeps across drills. A fact pays a gem the first time it reaches each level from 1 to 4, and a finished drill faster than the last with the same tables pays a bonus. The total only goes up and is never spent; the rule is in `docs/adr/0004-gem-rule.md`. "Coin" is not used.
- **Character**: the figure that keeps the learner company on every screen: the dragon, a cat, a robot, an owl, a unicorn or a monster. The dragon is there from the start and each of the others unlocks for good at a gem total. The learner chooses among the unlocked ones on the Start screen.
