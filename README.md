...
Renovate opens dependency update pull requests, including majors, waits until a release is 3 days old, and merges them with a merge commit once the `check` job passes, so the Playwright suite is the gate. Its pull requests carry the `internal` label, and the dependency dashboard issue lists what is pending.
