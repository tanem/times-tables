...
- **Badge pay**: the first drill after which every fact of a table has a highest level of 4 pays 10 gems for that table. A badge is derived from the highest levels, as an unlock is from earned, and needs no storing; the end screen finds the badges a drill earned by comparing before and after, as it finds unlocks.
- **A drill can cross two unlocks.** Facts pay at most 22, band pay 3, and a drill can complete more than one table at once, so a drill's pay is no longer under every gap between unlock totals. The unlock lookup returns every character crossed, in order, and the end screen shows a dialog for each.
