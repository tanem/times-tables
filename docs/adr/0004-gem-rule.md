...
- A document with a version higher than the build knows is not corrupt. The build leaves it and the backup key untouched, starts no drill, and shows one screen saying the app needs an update and to close and reopen it. The version 2 builds on installed copies do not have this rule, so ADR 0003's policy still covers this release: it is never rolled back, and a fault is fixed forward.
