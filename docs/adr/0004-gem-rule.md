...
- A finished drill that was faster than last time pays a bonus of 2 gems. Last time is the most recent finished drill with the same set of tables, in any order and however long ago. The drill is faster when its median answer time is under the earlier drill's. Both drills need a median and at least 10 right answers, a tie does not pay, and a quit drill neither pays nor counts as last time. The comparison reads that one earlier drill and looks no further back: when it has no median or too few right answers there is no bonus, because "Faster than last time!" has to be true of the drill it names. With no such earlier drill there is no comparison and no bonus. The comparison reads `median`, `tables`, `quit` and the outcome counts from the drill records, so it stores nothing new. Right answers are the fast and slow counts, which include answers at the cap, while the median leaves those out; a drill record does not hold how many were under the cap, and a drill answered mostly at the cap is not worth a stored figure. The end screen shows a faster drill as a race between a grey earlier self and the character, then "Faster than last time!", as the map settled.
...
- Once every character is unlocked and every fact has paid, nothing new unlocks. The total stays on the Start screen and rises only through the bonus.
- Erasing progress on the Parent view clears everything: a fresh document, no gems, the dragon chosen and every other character locked.
...
- The app reads version 3, and migrates a version 2 document that passes version 2's validation: each fact's `best` is set to its `level`, `gems` to the sum of those levels, and `character` to the dragon. Everything else is copied as it is. The migrated document is written back as version 3. The characters those gems unlock show as unlocked on the Start screen, with no dialog.
...
- A document with a version higher than the build knows is not corrupt. The build leaves it and the backup key untouched, starts no drill, and shows one screen saying the app needs an update and to close and reopen it. The build runs on a store that keeps nothing while that screen shows, so no save can reach the newer document. A service worker update may apply and reload on that screen as it may on the Start screen: ADR 0001 holds an update back so that a reload cannot end a drill, and no drill can follow this screen, so a fix arrives without a second close and reopen. The version 2 builds on installed copies do not have this rule, so ADR 0003's policy still covers this release: it is never rolled back, and a fault is fixed forward.

## Considered options

- **A gem for every fast answer.** The rule made up for the prototype on the `prototype/rewards-characters-sound` branch. A learner who drills only tables they know earns 20 a drill for ever.
...
- **A bonus of 5 gems, or 1.** The bonus is the only source without a ceiling, and a slow drill followed by a fast one earns it on purpose. At 5 it is most of what a late drill pays, and at two drills a day that trick alone earns a character every week or two. At 1 the race on the end screen pays a let-down. At 2 the trick costs 40 presentations for 2 gems, and lowers levels on the way.
- **The prototype's unlock totals, 25, 60, 120, 200 and 300, or 20, 50, 90, 140 and 200.** 300 sits 8 gems under what the facts can pay, and the last levels come slowly: the average learner below needs about 84 drills and the struggling learner more than 120. At 200 the average learner has every character in about 25 drills, under two weeks, which leaves little to aim for after that.
...
- **Announcing the characters unlocked at the migration in a dialog.** A screen that exists only for the few installed copies that migrate.
- **Erase keeping gems and characters.** Keeping gems while levels and highest levels reset would pay for every fact a second time. Keeping the highest levels too leaves an erased learner who can earn nothing. Letting the parent choose in the erase dialog is one more decision on a screen used rarely, and two paths to test.
...
- **A learner with three tables on** has 33 facts, which pay at most 132, and earns little after about 25 drills. Past that only the bonus pays, so in practice the unicorn and the monster need more tables switched on.
- **A learner who drills only tables they know** is that learner sooner: the facts pay once, within about 20 drills, and then nothing.
- **A learner who misses on purpose to climb again** earns nothing by it, because a fact's highest level reached never falls.
- **A learner who is simply good** earns faster and finishes sooner, against the same 308.
- A fact in the 2s and a fact in the 12s pay the same 4 gems, and a gem needs a fast outcome, which is judged against the learner's own pace, so the same effort earns about the same at any stage.
